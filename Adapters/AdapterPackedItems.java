...
public class AdapterPackedItems extends RecyclerView.Adapter<AdapterPackedItems.ListViewHolder> {
    ...
    public AdapterPackedItems(ArrayList<SingleListAplikator> exampleList, String tableName, boolean isTemplate, boolean online, String path) {
    ...
    public ListViewHolder onCreateViewHolder(ViewGroup parent, int i) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.recyclerview_packed_items, parent, false);
    ...
    public void onBindViewHolder(ListViewHolder listViewHolder, final int i) {
        ...
        if (isTemplate) {
            String currentItemPcs = (arrayList.get(i).getPcs() == 0) ? "Number of days" : arrayList.get(i).getPcs() + " pcs";
            listViewHolder.txtPcs.setText(currentItemPcs);

            listViewHolder.btnPack.setOnClickListener(v -> {
                if(online){
                    db.collection("templates").document(user.getEmail()).collection(tableName).document(arrayList.get(pos).getName()).delete();
                }else {
                    database.delete(tableName.replace(" ", "_"), "name = ?", new String[]{arrayList.get(pos).getName()});
                }
            ...
            listViewHolder.btnPack.setOnClickListener(v -> {
                //String itemName = arrayList.get(pos).getName();
                if (online) {
                    FirebaseFirestore db = FirebaseFirestore.getInstance();
                    if(path == null){
                        db.collection("lists").document(user.getEmail()).collection(tableName).document(arrayList.get(pos).getName()).update("packed", 0);
                    }else {
                        db.collection("lists").document(path).collection(tableName).document(arrayList.get(pos).getName()).update("packed", 0);
                    }
                }else {
